...
def display_data():
    ...
    iris_color = df['target'].map(color_map)
    fig = px.scatter(df, x=x0, y=x1, color='target2',hover_data=[x0, x1],)
    return fig
...
def make_df() -> pd.DataFrame:
    ...
    if st.session_state.plot_type == "blob":
        data, targets = datasets.make_classification(n_samples=params["n_samples"], n_informative=1, n_redundant=0, n_clusters_per_class=1, n_features=params["n_features"])
        n_features = params["n_features"]
    elif st.session_state.plot_type == "circle":
        data, targets = datasets.make_circles(n_samples=params["n_samples"], noise=params["noise"])
        n_features = 2
    elif st.session_state.plot_type == "moon":
        data, targets = datasets.make_moons(n_samples=params["n_samples"], noise=params["noise"])
        n_features = 2
    else:
    ...
    with col2:
        with st.container():
            ...
            if params["plot_type"] == "blob":
                ...
                c1.metric(label="number of samples", value=params["n_samples"], border=True)
                c2.metric(label="number of features", value=params["n_features"], border=True)
                c3.metric(label="class separator", value=params["class_sep"], border=True)
            else:
                c1, c2 = st.columns(2)
                c1.metric(label="number of samples", value=params["n_samples"], border=True)
